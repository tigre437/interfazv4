...
experiment = '240508_155950_A_WB_01ml B_WB_01ml'
...
path2 = os.path.join(experiment, 'time_series.csv')
...
for j in range(len(t_ext)-2):
    check_color_difference_A[:,j] = np.logical_and(abs(color_difference_A_norm[:,j]) > threshold,  mask == 1)
...
# for j in range(len(t_ext) - 2):
#     for i in range(96):
#         if check_color_difference_A[i,j] == 1:
#             color_difference_wells_A[i] = color_difference_A_norm[i,j]
...
print("Valores de NFrozen_A:", NFrozen_A)
print("Valor de N0_A:", N0_A)
print("Frozen fraction A:", frozen_fraction_A)


plt.figure(figsize=(8, 6))
plt.rcParams['font.size'] = 16
plt.plot(t_wells[2:],frozen_fraction_A, marker = '.', color = 'blue') 
plt.fill_betweenx(frozen_fraction_A, t_wells[2:]-error_t_wells[2:], t_wells[2:]+error_t_wells[2:], color = 'blue',alpha = 0.5)
plt.grid()
...
for j in range(len(t_ext)-2):
    check_color_difference_B[:,j] = np.logical_and(abs(color_difference_B_norm[:,j]) > threshold,  mask == 1)
...
# for j in range(len(t_ext) - 2):
#     for i in range(96):
#         if check_color_difference_B[i,j] == 1:
#             color_difference_wells_B[i] = color_difference_B_norm[i,j]
...
plt.fill_betweenx(frozen_fraction_B, t_wells[2:]-error_t_wells[2:], t_wells[2:]+error_t_wells[2:], color = 'blue',alpha = 0.5)
