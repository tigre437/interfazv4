...
def detect_circles(experiment, threshold, threshold_b, radio_min, radio_max):
    ...
    for cnt in contours:
        ...
        if len(approx)>=5:
            if cv2.isContourConvex(approx):
                ...
                if radius >=radio_min and radius <= radio_max: #pongo esto para que no considere el bloque
    ...
    print(len(t_ext)-2)
    print(len(check_color_difference_A[:,j]))
    print(len(color_difference_A_norm[:,j]))
